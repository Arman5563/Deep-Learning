...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        my_x=torch.clone(x)
        my_x=my_x.reshape(my_x.shape[0], -1)
        out= torch.matmul(my_x,w)+b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        my_x=torch.clone(x)
        my_x=torch.t(my_x.reshape(my_x.shape[0], -1)) #Also transposed
        dw=torch.matmul(my_x,dout)
        
        db=torch.sum(dout,0)
        
        w_copy=torch.t(torch.clone(w)) #Also transposed
        dx=torch.matmul(dout,w_copy).reshape(x.shape)
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        #####################################################
        # Replace "pass" statement with your code        
        dx=torch.mul(dout,0.5+0.5*torch.sign(x)) #Elementwise multiplication
        #####################################################
        #                  END OF YOUR CODE                 #
        #####################################################
        return dx
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        my_x=torch.clone(x)
        my_x=my_x.reshape(my_x.shape[0], -1)
        linear_out= torch.matmul(my_x,w)+b
        out=torch.maximum(linear_out,torch.zeros_like(linear_out))
        
        fc_cache = (x, w, b)
        relu_cache= linear_out
    ...
    def backward(dout, cache):
        ...
        dout_relu=torch.mul(dout,0.5+0.5*torch.sign(relu_cache))
        
        my_x=torch.clone(x)
        my_x=torch.t(my_x.reshape(my_x.shape[0], -1)) #Also transposed
        dw=torch.matmul(my_x,dout_relu)
        
        db=torch.sum(dout_relu,0)
        
        w_copy=torch.t(torch.clone(w)) #Also transposed
        dx=torch.matmul(dout_relu,w_copy).reshape(x.shape)
...
def softmax_loss(x, y):
    ...
    '''We know that it was not allowed, but there is a bug in setting the device
    in the notebook, and this .to() is used according to the instructions by the TA in the course Quera forum
    
    '''
    y_hat=y.expand(len(y),len(y)).t()
    x_hat=x-torch.max(x,1,keepdims=True)[0] #For nummerical stability of log_sum_exp
    
    #print(y_hat.device, x_hat.device, torch.mean(torch.logsumexp(x_hat,1)).device)
    loss=torch.mean(torch.logsumexp(x_hat,1))-torch.mean(torch.gather(x_hat,1,y_hat)[:,0])
    
    one_hot = x-x #creating zeros without using .to(device)
    one_hot.scatter_(1, y.unsqueeze(1), 1.0)
    
    dx=(torch.softmax(x_hat,1)-one_hot)/x.shape[0]
    ######################################################################
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        hidden_stds= weight_scale*torch.ones(input_dim, hidden_dim, dtype=dtype, device=device)
        hidden_means=torch.zeros(input_dim, hidden_dim, dtype=dtype, device=device)
        output_stds= weight_scale*torch.ones(hidden_dim, num_classes, dtype=dtype, device=device)
        output_means=torch.zeros(hidden_dim, num_classes, dtype=dtype, device=device)
        
        W1=torch.normal(hidden_means, hidden_stds).to(device).type(dtype)
        W2=torch.normal(output_means, output_stds).to(device).type(dtype)
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        lin_relu_out, lin_relu_cache = Linear_ReLU.forward(X,self.params['W1'],self.params['b1'])
        score_out, score_cache = Linear.forward(lin_relu_out,self.params['W2'],self.params['b2'])
        ...
        ###################################################################
        # Replace "pass" statement with your code
        smax_loss , smax_dx = softmax_loss(scores, y)
        
        reg_loss=torch.sum(torch.square(self.params['W1']))*self.reg+torch.sum(torch.square(self.params['W2']))*self.reg
        ## Only W are regularized
            
        loss= smax_loss+reg_loss
        
        dx2, dw2, db2 = Linear.backward(smax_dx, score_cache)
        _, dw1, db1 = Linear_ReLU.backward(dx2, lin_relu_cache)
        
        grads.update({'W1': 2*self.reg*self.params['W1']+dw1, 'W2': 2*self.reg*self.params['W2']+dw2, 'b1': db1, 'b2': db2})
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            layer_stds = weight_scale*torch.ones(input_dims[i], output_dims[i], dtype=dtype, device=device)
            layer_means = torch.zeros(input_dims[i], output_dims[i], dtype=dtype, device=device)
            
            Wi=torch.normal(layer_means, layer_stds).to(device).type(dtype)
            bi=torch.zeros(output_dims[i], dtype=dtype, device=device)

            self.params.update({'W'+str(i+1):Wi, 'b'+str(i+1):bi})
    ...
    def loss(self, X, y=None):
        ...
        if self.use_dropout:
            for i in range(self.num_layers-1):
                lin_relu_out, lin_relu_cache = Linear_ReLU.forward(lin_relu_in,self.params['W'+str(i+1)],self.params['b'+str(i+1)])
                
                
                dropout_out, dropout_cache=Dropout.forward(lin_relu_out, self.dropout_param)
                lin_relu_in=dropout_out
                
                cache_list.append( (lin_relu_cache, dropout_cache) )
        else:
            for i in range(self.num_layers-1):
                lin_relu_out, lin_relu_cache = Linear_ReLU.forward(lin_relu_in,self.params['W'+str(i+1)],self.params['b'+str(i+1)])
                cache_list.append(lin_relu_cache)
                lin_relu_in=lin_relu_out
        
        score_out, score_cache = Linear.forward(lin_relu_out,self.params['W'+str(self.num_layers)],self.params['b'+str(self.num_layers)])
        ...
        reg_loss=0
        for i in range(self.num_layers):
            reg_loss += torch.sum(torch.square( self.params['W'+str(i+1)] ))*self.reg/2
        ## Only W are regularized
            
        loss= smax_loss+reg_loss
        
        dxi, dwi, dbi = Linear.backward(smax_dx, score_cache)
        grads.update({'W'+str(self.num_layers): self.reg*self.params['W'+str(self.num_layers)]+dwi, 'b'+str(self.num_layers): dbi})
        
        if self.use_dropout:
            for i in range(self.num_layers-2,-1,-1):
                lin_relu_cache, dropout_cache = cache_list[i]
                
                dxd=Dropout.backward(dxi, dropout_cache)
                
                dxi, dwi, dbi = Linear_ReLU.backward(dxi, lin_relu_cache)
                
                dxi,_,_ = Linear_ReLU.backward(dxd, lin_relu_cache)
                grads.update({'W'+str(i+1): self.reg*self.params['W'+str(i+1)]+dwi, 'b'+str(i+1): dbi})  
        else:
            for i in range(self.num_layers-2,-1,-1):
                dxi, dwi, dbi = Linear_ReLU.backward(dxi, cache_list[i])
                grads.update({'W'+str(i+1): self.reg*self.params['W'+str(i+1)]+dwi, 'b'+str(i+1): dbi})
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def rmsprop(w, dw, config=None):
    ...
    config['cache'] += (1-config['decay_rate'])*(-config['cache'] + torch.square(dw))
...
def adam(w, dw, config=None):
    ...
    next_w=w-mhat*config['learning_rate']/(torch.sqrt( vhat) + config['epsilon'] )
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            rand_vs=torch.rand(x.size())
            mask=torch.zeros_like(x)
            if p<1:
                mask[rand_vs>p]=1/(1-p)
